...
export const Route = createFileRoute("/_authenticated/integrations/mercado-livre/callback")({
  head: () => ({ meta: [{ title: "Conectando Mercado Livre…" }] }),
  validateSearch: (s: Record<string, unknown>) => ({
    code: typeof s.code === "string" ? s.code : undefined,
    state: typeof s.state === "string" ? s.state : undefined,
    error: typeof s.error === "string" ? s.error : undefined,
  }),
  component: MercadoLivreCallback,
});
...
function MercadoLivreCallback() {
  ...
  useEffect(() => {
    ...
    (async () => {
      if (oauthError) { setStatus("error"); setMessage(`Mercado Livre retornou erro: ${oauthError}`); return; }
      if (!code) { setStatus("error"); setMessage("Sem code na URL."); return; }

      const pendingRaw = sessionStorage.getItem(STORAGE_KEY);
      if (!pendingRaw) {
        setStatus("error");
        setMessage("Sessão expirada. Reabra a página de Mercado Livre e tente de novo.");
        return;
      }
      ...
      try {
        ...
        const { data: exch, error: exchErr } = await supabase.functions.invoke("mercado-livre-oauth-exchange", {
          body: { code, client_id: pending.client_id, client_secret: pending.client_secret, redirect_uri: pending.redirect_uri },
        });
        ...
        const { error: pushErr } = await supabase.functions.invoke("mercado-livre-push-tokens", {
          body: {
        ...
        setTimeout(() => navigate({ to: "/integrations/mercado-livre" }), 1500);
      } catch (e) {
        setStatus("error");
        setMessage(e instanceof Error ? e.message : String(e));
        toast.error("Falha ao conectar Mercado Livre");
      }
    })();
