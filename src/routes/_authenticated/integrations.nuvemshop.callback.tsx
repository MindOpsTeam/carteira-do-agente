...
function NuvemshopCallback() {
  ...
  useEffect(() => {
    ...
    (async () => {
      if (oauthError) { setStatus("error"); setMessage(`Nuvemshop retornou erro: ${oauthError}`); return; }
      if (!code) { setStatus("error"); setMessage("Sem code na URL."); return; }

      const pendingRaw = sessionStorage.getItem(STORAGE_KEY);
      if (!pendingRaw) {
        setStatus("error");
        setMessage("Sessão expirada. Reabra a página de Nuvemshop e tente de novo.");
        return;
      }
      const pending = JSON.parse(pendingRaw) as {
        client_id: string; client_secret: string; instance_id: string;
      };
      ...
      try {
        ...
        toast.success("Nuvemshop conectado!");
        setTimeout(() => navigate({ to: "/integrations/nuvemshop" }), 1500);
      } catch (e) {
        setStatus("error");
        setMessage(e instanceof Error ? e.message : String(e));
        toast.error("Falha ao conectar Nuvemshop");
      }
    })();
