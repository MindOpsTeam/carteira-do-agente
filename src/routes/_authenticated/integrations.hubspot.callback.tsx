...
function HubspotCallback() {
  ...
  useEffect(() => {
    ...
    (async () => {
      if (oauthError) {
        setStatus("error");
        setMessage(`HubSpot retornou erro: ${oauthError}`);
        return;
      }
      if (!code) {
        setStatus("error");
      ...
      if (!pendingRaw) {
        setStatus("error");
        setMessage("Sessão expirada. Reabra a página de HubSpot e tente de novo.");
        return;
      }
      ...
      try {
        const { data, error } = await supabase.functions.invoke("hubspot-oauth-exchange", {
          body: {
            code,
        ...
        setStatus("ok");
        setMessage("HubSpot conectado com sucesso!");
        toast.success("HubSpot conectado!");
        setTimeout(() => navigate({ to: "/integrations" }), 1500);
      } catch (e) {
        setStatus("error");
        setMessage(e instanceof Error ? e.message : String(e));
        toast.error("Falha ao conectar HubSpot");
      }
    })();
