...
function BlingCallback() {
  ...
  useEffect(() => {
    ...
    (async () => {
      if (oauthError) {
        setStatus("error");
        setMessage(`Bling retornou erro: ${oauthError}`);
        return;
      }
      if (!code) {
        setStatus("error");
      ...
      if (!pendingRaw) {
        setStatus("error");
        setMessage("Sessão expirada. Reabra a página de Bling e tente de novo.");
        return;
      }
      ...
      try {
        ...
        const { data: exch, error: exchErr } = await supabase.functions.invoke("bling-oauth-exchange", {
          body: {
            code,
        ...
        const { error: pushErr } = await supabase.functions.invoke("bling-push-tokens", {
          body: {
        ...
        setTimeout(() => navigate({ to: "/integrations/bling" }), 1500);
      } catch (e) {
        setStatus("error");
        setMessage(e instanceof Error ? e.message : String(e));
        toast.error("Falha ao conectar Bling");
      }
    })();
