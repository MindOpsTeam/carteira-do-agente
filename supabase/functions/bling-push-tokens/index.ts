/**
 * POST /bling-push-tokens
 * Empurra os tokens Bling pra VPS (escreve em ~/.agente-cfo/.env via OpenClaw).
 * Atualiza instances.connected_integrations.bling.
 *
...
Deno.serve(async (req: Request) => {
  ...
  // Comando que escreve as credenciais no .env da VPS via openclaw
  const command = [
    "Execute em sequência:",
    `1) mkdir -p ~/.agente-cfo`,
    `2) Atualize ~/.agente-cfo/.env definindo (sobrescreva se já existir):`,
    `BLING_CLIENT_ID=${body.client_id}`,
    `BLING_CLIENT_SECRET=${body.client_secret}`,
  ...
  try {
    ...
    await supabase.from("audit_log").insert({
      ...
      action: "bling_push_tokens_failed",
