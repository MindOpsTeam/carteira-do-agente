/**
 * POST /nuvemshop-push-tokens
 * Empurra tokens Nuvemshop pra VPS e atualiza instances.connected_integrations.
 */
...
Deno.serve(async (req: Request) => {
  ...
  let body: {
    instance_id?: string; access_token?: string; user_id?: number | string;
    client_id?: string; client_secret?: string;
  };
  try { body = await req.json(); } catch { return errorResponse("Body JSON inválido", 400); }

  if (!body.instance_id || !body.access_token || !body.user_id || !body.client_id || !body.client_secret) {
    return errorResponse("instance_id/access_token/user_id/client_id/client_secret obrigatórios", 400);
  }
  ...
  const command = [
    ...
    `NUVEMSHOP_CLIENT_ID=${body.client_id}`,
    `NUVEMSHOP_CLIENT_SECRET=${body.client_secret}`,
    `NUVEMSHOP_ACCESS_TOKEN=${body.access_token}`,
    `NUVEMSHOP_STORE_ID=${body.user_id}`,
  ...
  try {
    ...
    await supabase.from("audit_log").insert({
      actor_user_id: user.id, action: "nuvemshop_push_tokens_failed",
  ...
  const updated = {
    ...current,
    nuvemshop: { connected_at: new Date().toISOString(), store_id: body.user_id },
  };
  await supabase.from("instances").update({ connected_integrations: updated }).eq("id", body.instance_id);

  await supabase.from("audit_log").insert({
    actor_user_id: user.id, action: "nuvemshop_push_tokens",
    payload: { instance_id: body.instance_id, store_id: body.user_id },
  });
