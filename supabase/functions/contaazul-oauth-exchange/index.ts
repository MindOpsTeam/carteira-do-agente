/**
 * POST /contaazul-oauth-exchange
 * Troca o authorization code do ContaAzul por access_token + refresh_token.
...
Deno.serve(async (req: Request) => {
  ...
  let respText: string;
  try {
    resp = await fetch("https://api.contaazul.com/oauth2/token", {
      method: "POST",
      headers: {
    ...
    return errorResponse(`Falha ao contatar ContaAzul: ${String(err)}`, 502);
  }

  if (!resp.ok) {
    return errorResponse(`ContaAzul retornou ${resp.status}: ${respText}`, 502);
  }

  let tokens: { access_token?: string; refresh_token?: string; expires_in?: number; token_type?: string };
  try { tokens = JSON.parse(respText); } catch { return errorResponse("Resposta do ContaAzul não é JSON", 502); }

  if (!tokens.access_token || !tokens.refresh_token) {
    return errorResponse("ContaAzul não retornou tokens", 502);
  }
